...
<div
    class="mt-6 flex min-h-0 flex-1 flex-col gap-5"
    ...
    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-gray-500">Ringkasan capaian Al-Quran dan Hadits anak Anda.</p>
            </div>
        </div>
        <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Total Modul</p>
                <p class="mt-2 text-3xl font-semibold text-gray-900" x-show="activeCategory === 'al-quran'">{{ $quranStats['total'] }}</p>
                <p class="mt-2 text-3xl font-semibold text-gray-900" x-show="activeCategory === 'al-hadits'">{{ $haditsStats['total'] }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Selesai</p>
                <p class="mt-2 text-3xl font-semibold text-emerald-600" x-show="activeCategory === 'al-quran'">{{ $quranStats['completed'] }}</p>
                <p class="mt-2 text-3xl font-semibold text-emerald-600" x-show="activeCategory === 'al-hadits'">{{ $haditsStats['completed'] }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Sedang Dikerjakan</p>
                <p class="mt-2 text-3xl font-semibold text-orange-500" x-show="activeCategory === 'al-quran'">{{ $quranStats['inProgress'] }}</p>
                <p class="mt-2 text-3xl font-semibold text-orange-500" x-show="activeCategory === 'al-hadits'">{{ $haditsStats['inProgress'] }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Rata-rata Pencapaian</p>
                <p class="mt-2 text-3xl font-semibold text-slate-900" x-show="activeCategory === 'al-quran'">{{ $quranStats['average'] }}%</p>
                <p class="mt-2 text-3xl font-semibold text-slate-900" x-show="activeCategory === 'al-hadits'">{{ $haditsStats['average'] }}%</p>
            </div>
        </div>
    </div>

    <div class="grid min-h-0 flex-1 gap-5 xl:grid-cols-[1fr_320px]">
        <div class="flex h-full min-h-[460px] flex-col rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <p class="text-sm text-gray-500">Ringkasan Progress</p>
                    <h3 class="text-xl font-semibold text-gray-900" x-show="activeCategory === 'al-quran'">Materi Al-Quran</h3>
                    <h3 class="text-xl font-semibold text-gray-900" x-show="activeCategory === 'al-hadits'">Materi Al-Hadits</h3>
                    <p class="text-sm text-gray-500">Progress halaman dan jumlah juz ditampilkan per modul.</p>
                </div>
            ...
            <div class="mt-5 flex-1 overflow-hidden rounded-xl border border-gray-200 bg-white">
                <div class="grid grid-cols-[minmax(120px,165px)_minmax(145px,170px)_1fr] items-center border-b border-gray-200 bg-gray-50 px-4 py-2 text-[10px] font-semibold uppercase text-gray-600">
                    ...
                    @foreach($quranPages as $pageIndex => $pageItems)
                        ...
                        >
                            @foreach($pageItems as $entry)
                                <div class="grid grid-cols-[minmax(120px,165px)_minmax(145px,170px)_1fr] items-center gap-3 px-4 py-3 hover:bg-gray-50/80">
                                    <p class="text-sm font-medium text-gray-900">{{ $entry['judul'] }}</p>
                                    <p class="inline-flex items-baseline gap-1 text-sm font-semibold text-gray-800 tabular-nums">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="inline-block w-[5ch] text-right text-sm font-semibold text-gray-800 tabular-nums">{{ $entry['persentase'] }}%</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="mt-4 flex items-center justify-between gap-3">
                <button
                    ...
                    class="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                <button
                    ...
                    class="inline-flex items-center gap-2 rounded-xl border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Next
        ...
        <div class="flex h-full min-h-[460px] flex-col rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900">Update Terbaru</h3>
                </div>
            ...
            <div class="mt-4 flex-1 overflow-y-auto pr-1">
                <ul class="space-y-3" x-show="activeCategory === 'al-quran'">
                    @forelse($quranUpdates as $entry)
                        <li class="rounded-xl border border-gray-100 p-3">
                            <p class="text-sm font-semibold text-gray-900">{{ $entry['judul'] }}</p>
                ...
                <ul class="space-y-3" x-show="activeCategory === 'al-hadits'">
