...
@php
    ...
    $recent = $data->take(5);
    $issues = $data->filter(fn ($row) => in_array($row->status, ['izin','alpa','telat']))->take(4);
@endphp
...
<div class="mt-6 space-y-6">
    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                <p class="text-sm text-gray-500">Izin/Terlambat</p>
                <p class="mt-2 text-3xl font-semibold text-amber-500">{{ $izin + $telat }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4">
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Konfirmasi kepada musyrif jika diperlukan.</p>
                </div>
                <span class="text-xs text-gray-500">{{ $issues->count() }} catatan</span>
            </div>
            <ul class="mt-4 space-y-3">
